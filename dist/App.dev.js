...
var todoController = function () {
  var Todo = function Todo(id, description) {
    this.id = id;
  ...
  return {
    addItem: function addItem(desc) {
      ...
      newItem = new Todo(ID, desc); // Push it into our data structure

      data.allItems.push(newItem); // Return the new element
...
var UIController = function () {
  ...
  return {
    ...
    addListItem: function addListItem(obj) {
      ...
      html = '<div class="item" id="will-%id%"><div class="right floated content"><button class="ui inverted red button deleteButton"><i class="trash alternate icon"></i></button></div><div class="content"><div class="header todo">%description% (%time%)</div></div></div>'; // Replace the placeholder text with some actual data

      newHtml = html.replace('%id%', obj.id);
      newHtml = newHtml.replace('%description%', obj.description);
      newHtml = newHtml.replace('%time%', obj.time); // Insert the HTML into the DOM
...
var controller = function (todoCtrl, UICtrl) {
  ...
  var ctrlDeleteItem = function ctrlDeleteItem(event) {
    ...
    if (itemID) {
      ...
      todoCtrl.deleteItem(ID); // 2. Delete the item from the UI
  ...
  var ctrlAddItem = function ctrlAddItem() {
    ...
    if (input !== "") {
      ...
      newItem = todoCtrl.addItem(input); // 3. Add the item to the UI

      UICtrl.addListItem(newItem); // 4. Clear the fields
